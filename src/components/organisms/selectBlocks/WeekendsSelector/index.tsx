...
const WeekendsSelector: FC<IProps> = ({ onNext, onBack, progress }) => {
  ...
  const handleOption = (weekend: string) => {
    dispatch(
      setQuestionnaire({
        ...state,
        children: { ...state.children, weekend },
        step: progress + 1,
      }),
    );
  ...
  return (
    <MainContainer background="#eef3f9">
      <Box>
        <SelectorHeader
          title={localization[ELocalizationQuestionnaire.QUEST_WEEKEND_TITLE]}
          progressTitle={
        ...
        <Box className={styles.selectorWrapper}>
          {DEFAULT_WEEKENDS_DATA.map((el) => (
            <SelectorOption
              key={el.id}
              icon={el.icon}
              title={localization[el.title]}
              onClick={() => handleOption(el.value || "")}
              isActive={state.children.weekend === el.value}
            />
          ))}
        </Box>
      </Box>
