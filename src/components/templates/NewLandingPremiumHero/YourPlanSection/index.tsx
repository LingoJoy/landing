...
const PlanCard: FC<ICardProps> = ({ data, active, discount }) => {
  const priceForrmated = discount > 0 ? parseCurrency(`${data.periodPrice}`) : parseCurrency(`${data.periodPriceWithoutDiscount}`);
                    ...
                    {loading ? (
                      ...
                      plans.map((el) => (
                        <Box key={el.id} onClick={() => onPlan(el)}>
                          <PlanCard data={el} active={plan.id === el.id} discount={discount} />
                        </Box>
                      ))
                    )}
                  </>
